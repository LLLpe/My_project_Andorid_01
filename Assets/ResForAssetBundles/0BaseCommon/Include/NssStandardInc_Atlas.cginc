...
#ifndef NSS_STANDARD_INCLUDED_ATLAS
#define  NSS_STANDARD_INCLUDED_ATLAS
...
//// for per-pixel boundary test
//float _EnableReflectionPlaneBounds;		// 1.0 - Enable, 0.0 - Disable
//float _ReflectionPlaneRadius;
//float3 _ReflectionPlaneCenter;
#endif
...
UNITY_INSTANCING_BUFFER_START(PropsStandard)
UNITY_DEFINE_INSTANCED_PROP(half4, _BaseColor)
UNITY_INSTANCING_BUFFER_END(PropsStandard)
...
#endif
	// atlas bound
	float2 xBound : TEXCOORD2;		// uv3: [xMin, xMax]
	float2 yBound : TEXCOORD3;		// uv4: [yMin, yMax]
	float2 scale : TEXCOORD4;		// x = texture scaling factor (atlas / original size)
...
#endif
	float4 uv2 : TEXCOORD10;	// uv bound on Atlas
	float uv3 : TEXCOORD11;
	UNITY_VERTEX_INPUT_INSTANCE_ID
};
...
inline half3 GI_IndirectSpecular(GIInput data, half occlusion, GlossyEnvironmentData glossIn)
{
	...
	//// if the pixel (worldPos) is inside the bounding sphere, texture from _MirrorReflectionTex
	//if (_EnableReflectionPlaneBounds > 0.5){
	//	float dist = distance(data.worldPos, _ReflectionPlaneCenter);

	//	if (dist <= _ReflectionPlaneRadius){
	//		specular = tex2D(_MirrorReflectionTex, data.screenProjCoord).rgba;
	//	}
	//}
	//else {
...
#ifdef MORE_HALF
//half3 Emission(half2 uv, float2 originalUV0)
half3 Emission(half2 uv, float2 dx, float2 dy)
#else
//half3 Emission(float2 uv, float2 originalUV0)
half3 Emission(float2 uv, float2 dx, float2 dy)
#endif
{
#ifdef _EMISSION
	return tex2D(_EmissionMap, uv, dx, dy).rgb * _EmissionColor.rgb * _EmissionScale;
	//return tex2D(_EmissionMap, uv, ddx(originalUV0), ddy(originalUV0)).rgb * _EmissionColor.rgb * _EmissionScale;
...
#ifdef MORE_HALF
//inline FragmentCommonData NormalAndMetallicSetup(half4 i_tex, half2 i_tex2, half3 i_eyeVec, float3 i_posWorld, half bumpScale, half4 i_color, half4 tangentToWorld[3], float2 originalUV0)
inline FragmentCommonData NormalAndMetallicSetup(half4 i_tex, half2 i_tex2, half3 i_eyeVec, float3 i_posWorld, half bumpScale, half4 i_color, half4 tangentToWorld[3], float2 dx, float2 dy)
#else
//inline FragmentCommonData NormalAndMetallicSetup(float4 i_tex, float2 i_tex2, float3 i_eyeVec, float3 i_posWorld, half bumpScale, half4 i_color, float4 tangentToWorld[3], float2 originalUV0)
inline FragmentCommonData NormalAndMetallicSetup(float4 i_tex, float2 i_tex2, float3 i_eyeVec, float3 i_posWorld, half bumpScale, half4 i_color, float4 tangentToWorld[3], float2 dx, float2 dy)
#endif
{
	FragmentCommonData o = (FragmentCommonData)0;

	half4 baseColor = UNITY_ACCESS_INSTANCED_PROP(PropsStandard, _BaseColor);
	...
	//half4 normalMR = tex2D(_NormalMRMap, i_tex.xy);
	//half4 normalMR = tex2D(_NormalMRMap, i_tex.xy, ddx(originalUV0), ddy(originalUV0));
	half4 normalMR = tex2D(_NormalMRMap, i_tex.xy, dx, dy);
	...
	//half4 albedo = tex2D(_BaseMap, i_tex.xy, ddx(originalUV0), ddy(originalUV0));	// apply tex2DGrad => to handle visible seams
	half4 albedo = tex2D(_BaseMap, i_tex.xy, dx, dy);	// apply tex2DGrad => to handle visible seams
...
#ifdef MORE_HALF
//inline FragmentCommonData FragmentSetupTangent(inout half4 i_tex, half2 i_tex2, half3 i_eyeVec, half4 tangentToWorld[3], float3 i_posWorld, half bumpScale, half4 i_color, float2 originalUV0)
inline FragmentCommonData FragmentSetupTangent(inout half4 i_tex, half2 i_tex2, half3 i_eyeVec, half4 tangentToWorld[3], float3 i_posWorld, half bumpScale, half4 i_color, float2 dx, float2 dy)
#else
//inline FragmentCommonData FragmentSetupTangent(inout float4 i_tex, float2 i_tex2, float3 i_eyeVec, float4 tangentToWorld[3], float3 i_posWorld, half bumpScale, half4 i_color, float2 originalUV0)
inline FragmentCommonData FragmentSetupTangent(inout float4 i_tex, float2 i_tex2, float3 i_eyeVec, float4 tangentToWorld[3], float3 i_posWorld, half bumpScale, half4 i_color, float2 dx, float2 dy)
#endif
{
	//FragmentCommonData o = NormalAndMetallicSetup(i_tex, i_tex2, i_eyeVec, i_posWorld, bumpScale, i_color, tangentToWorld, originalUV0);
	FragmentCommonData o = NormalAndMetallicSetup(i_tex, i_tex2, i_eyeVec, i_posWorld, bumpScale, i_color, tangentToWorld, dx, dy);
	return o;
}
