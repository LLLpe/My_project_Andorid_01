...
half4 _HeightMaskTex_ST;
half _HeightOffset;
half _HeightContrast;
half _VertexColorPower;
CBUFFER_END
...
#if defined(_SECOND_LAYER) || defined(_HEIGHT_MASK)
	float4 tex2	: TEXCOORD9;
#endif
#if SPLATOON
	float2 uv2_SplatTex	: TEXCOORD10;
#endif
	UNITY_VERTEX_INPUT_INSTANCE_ID
};
...
#ifdef MORE_HALF
inline FragmentCommonData NormalAndMetallicSetup(half4 i_tex, half4 i_tex2, half3 i_eyeVec, float3 i_posWorld, half bumpScale, half4 i_color, half4 tangentToWorld[3])
#else
inline FragmentCommonData NormalAndMetallicSetup(float4 i_tex, float4 i_tex2, float3 i_eyeVec, float3 i_posWorld, half bumpScale, half4 i_color, float4 tangentToWorld[3])
#endif
{
	...
	half alphaMask = i_color.a;
#ifdef _HEIGHT_MASK
	half height = tex2D(_HeightMaskTex, i_tex2.zw).x;
	half heightBottom = (height * 0.5 + 0.5) * 2.0 * (1.0 - max(_HeightOffset, 0.5));
	alphaMask *= _HeightOffset * 2.0;
	alphaMask = 1.0 - saturate(saturate(max(alphaMask, heightBottom) - alphaMask) / (1.0 - min(_HeightContrast, 0.999)));
#endif
	...
	o.alpha = albedo.a * baseColor.a * alphaMask;
#endif
...
#ifdef MORE_HALF
inline FragmentCommonData FragmentSetupTangent(inout half4 i_tex, half4 i_tex2, half3 i_eyeVec, half4 tangentToWorld[3], float3 i_posWorld, half bumpScale, half4 i_color)
#else
inline FragmentCommonData FragmentSetupTangent(inout float4 i_tex, float4 i_tex2, float3 i_eyeVec, float4 tangentToWorld[3], float3 i_posWorld, half bumpScale, half4 i_color)
#endif
{
