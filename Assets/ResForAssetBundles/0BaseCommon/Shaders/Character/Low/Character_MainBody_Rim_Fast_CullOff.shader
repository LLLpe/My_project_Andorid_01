...
Shader "QF/Character/Low/Character_MainBody_Rim_Fast_CullOff" 
{
...
CBUFFER_END
			...
			half4 frag (VertexOutput i) : COLOR
			{
				...
				if(saeMask.r > 0.1)
				{
					reflectMapColor = sqrt(reflectMapColor);
				}
				half3 MainRampColor = lerp(tex2D(_RampMap, rampUVCloth), tex2D(_RampMap, rampUVSkin), saeMask.r) * mainTexColor.rgb;
				UNITY_BRANCH
				if(saeMask.r > 0.1)
				{
					MainRampColor = pow(MainRampColor, 0.454);
				}

				half3 baseColor = MainRampColor *  i.vlight ;
				...
				return NSS_CALC_FOG(i, NSS_OUTPUT_COLOR_SPACE_CHARACTER(half4(finalColor,1.0)));
