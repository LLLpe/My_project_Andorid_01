...
half _EmissionScale2;
half4 _EmissionColor2;
#endif

#ifdef _AA_ON
half _SpecularAntiAliasingScreenSpaceVariance;
...
#ifdef _TRIPLANAR_DISSOLVE_ON
	float3 posNormal                 : TEXCOORD9;
#endif
	UNITY_VERTEX_INPUT_INSTANCE_ID
};
...
#ifdef _INT_REFLECTION_CUBLE_LEVEL
	half4 rgbm = SAMPLE_TEXTURECUBE_LOD(tex, samplertex, R, (int)mip);//采样一级效率更高
#else
...
inline half3 GI_IndirectSpecular(GIInput data, half occlusion, GlossyEnvironmentData glossIn)
{
...
#ifdef _PLANAR_REFLECTION_ON
	specular = tex2D(_MirrorReflectionTex, data.screenProjCoord).rgb;
#if !_ENABLETOD
	...
	specular = lerp(env0,specular,(1 - glossIn.roughness) * (1 - glossIn.roughness));
#else
#if !_ENABLETOD
	half3 env0 = GlossyEnvironment(TEXTURECUBE_ARGS(unity_SpecCube0,samplerunity_SpecCube0), data.probeHDR[0], glossIn);
#else
...
#ifdef MORE_HALF
half3 Emission(half2 uv, half lerpScale)
#else
half3 Emission(float2 uv, float lerpScale)
#endif
{
#ifdef _EMISSION
#ifdef _SECOND_LAYER
	half3 emission = tex2D(_EmissionMap, uv).rgb * _EmissionColor.rgb * _EmissionScale;
	emission = lerp(tex2D(_EmissionMap2, uv).rgb * _EmissionColor2.rgb * _EmissionScale2, emission, lerpScale);
	return emission;
#else
...
#endif
{
	...
	half2 metallicRoughness = half2(metallic, roughness);
		...
		half3 normalTangent = NssUnpackScaleNormalXY(normalMR.xy, bumpScale);
		o.normalWorld = PerPixelWNormal(normalTangent, tangentToWorld);
		o.normalWorld = lerp(normalize(tangentToWorld[0]), o.normalWorld, i_color.a);
		#else
		o.normalWorld = normalize(tangentToWorld[0]);
		#endif 
		...
		o.normalWorld = PerPixelWNormal(normalTangent, tangentToWorld);
		o.normalWorld = lerp(o.normalWorld, normalize(tangentToWorld[0]), i_color.a);
		#else
		o.normalWorld = normalize(tangentToWorld[0]);
		#endif
	#else
	...
	#endif
#ifdef _AA_ON
	roughness = saturate(GeometricNormalFiltering(roughness, o.normalWorld, _SpecularAntiAliasingScreenSpaceVariance, _SpecularAntiAliasingThreshold));
#endif
...
#ifdef _ENABLE_ALPHABLEND
	o.alpha = albedo.a * baseColor.a;
#elif _ENABLE_ALPHAPREMULTIPLY
	o.diffColor *= o.alpha;
...
#endif
{
  ...
  //  sampler2D unity_DitherMask;
    real UnityApplyDitherCrossFadeValue(half2 vpos)
    {
        ...
        vpos /= 4; // the dither mask texture is 4x4
        real mask = tex2D(unity_DitherMask, vpos).a;
        real sgn = unity_LODFade.x > 0 ? 1.0f : -1.0f;
        return (1 - step(0,(unity_LODFade.x - mask * sgn)));		
    }
#else
