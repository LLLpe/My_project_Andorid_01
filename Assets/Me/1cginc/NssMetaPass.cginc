...
half4 UnityMeta_pbrAlbedo(UnityMetaInput IN)
{
	...
	if (IsGammaSpace())
	{
		baseColor = half3(SRGBToLinear(baseColor.x), SRGBToLinear(baseColor.y), SRGBToLinear(baseColor.z)); //FastSRGBToLinear(baseColor);
		SpecularColor = FastSRGBToLinear(SpecularColor);
	}
...
half4 UnityMeta_pbrMetalspec(UnityMetaInput IN)
{
	...
	if (IsGammaSpace())
	{
		baseColor.xyz = FastSRGBToLinear(baseColor.xyz);
		SpecularColor = FastSRGBToLinear(SpecularColor);
	}
...
half4 UnityMetaFragment(UnityMetaInput IN)
{
	...
	if (unity_MetaFragmentControl.y)
	{
		...
		if (unity_UseLinearSpace)
			...
			emission = FastSRGBToLinear(IN.Emission);

#if USE_UE_SHADING
		res = half4(3.14159265358979323846f * emission, 1.0);
#else
