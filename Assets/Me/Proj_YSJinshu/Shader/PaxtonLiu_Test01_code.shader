Shader "PaxtonLiu/HLSL_Test01"
{
    Properties
    {
        [MainTexture]_MainTex("Color" , 2D) = "white"{}
        [Maincolor][HDR]_ColorTint("Tint" , color) = (1,1,1,1)
        ...
        _CrystalHeight1("CrystalHeight1"  ,Range( -10 , 10)) = 1
        _CrystalTilling1("_CrystalTilling1"  ,Range( 0, 10)) = 1
        ...
        _GlitterDensityMap("密度图", 2D) = "white" {}
		_GlitterDensity("GlitterDensity", float) = 1
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _GlitterDensityMap;
	        half4 _GlitterDensityMap_ST;
            ...
            half4 frag (v2f i) : SV_TARGET
            {
                ...
                half3 tsViewDir = mul(tangent2World, vDirWS).xyz;//parallaxUV
                half3 normalTS = normalize(mul(TS_TexNormal,tangent2World));
                
                half3 ViewN =  i.normalVS_uv;
                ViewN.xy += TS_TexNormal.xy * _Bump;
                half NdotL = dot(normalTS,normalize(light.direction));
                NdotL = max(0,NdotL);
                
                half NdotV = dot(normalTS,vDirWS);
                half NdotH = dot(normalTS,normalize(light.direction + vDirWS));
                NdotH = max(0,NdotH);
                half NdotVOffset = dot(normalTS,normalize(light.direction + vDirWS +  half3(_UvOffsetX,_UvOffsetY,_UvOffsetZ)));
                ...
                half3 SpecularColor = lerp(half3(0.3, 0.3, 0.3), baseColor,  0) * lightMap.b;
                
                //BaseColor
                half3 RampColor = NPR_Base_Ramp (NdotL , _RampTex, lightMap, _RampRange);
                half3 Albedo = baseColor * RampColor;
                //Albedo = baseColor ;
                //Albedo *= (1 - step(0.95,lightMap.r))* _AlbedoIntensity ;

                //MetaMap镜面反射
                half3 Specular = NPR_Base_Specular(NdotL,NdotH,_RampTexSpec,baseColor,lightMap,MetalFactor);
                ...
                //NPR菲涅尔
                half3 Fresnel = NPR_Base_RimLight(NdotV,NdotL,baseColor);
                ...
                half2 uv_GlitterDensityMap = i.uv * _GlitterDensityMap_ST.xy + _GlitterDensityMap_ST.zw;
                half GlitterDensity = tex2D(_GlitterDensityMap,uv_GlitterDensityMap);
                GlitterDensity *= _GlitterDensity;
                //parallaxMap
                half2 uv3_CrystalTex = i.uv * _CrystalMap_ST.xy + _CrystalMap_ST.zw;
                half2 CrystalUV = ((uv3_CrystalTex + (_CrystalHeight1 * (tsViewDir).xy * 0.01)) * _CrystalTilling1 * GlitterDensity);
                half3 CrystalCol = tex2D(_CrystalMap,CrystalUV.xy);
                half4 finalcol = 1;
                half4 Test = 1;
                Test.rgb = CrystalCol  ;
